...
class Generator(nn.Module):
    ...
    def forward(self, input):
        ...
        # mask = Variable(torch.ones(batch_size, 1, row, col)).to(torch.device("cuda:1")) / 2.
        # h = Variable(torch.zeros(batch_size, 32, row, col)).to(torch.device("cuda:1"))
        # c = Variable(torch.zeros(batch_size, 32, row, col)).to(torch.device("cuda:1"))
        mask_list = []
